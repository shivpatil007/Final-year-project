...
db = create_engine("postgresql://postgres:abcd@localhost:5433/proj_db")
...
def send_notification(sender):
    ...
    for x in raw:
        response = sendPostRequest(
            URL, 'CX97133NP0QNNM9MSNSE0QR7A05GS72A', 'MDV7RXBWBZL593EX', 'stage', x, '1', msg)
...
def send_otp():
    if request.method == 'POST':
        ...
        # get response
        response = sendPostRequest(
            URL, 'CX97133NP0QNNM9MSNSE0QR7A05GS72A', 'MDV7RXBWBZL593EX', 'stage', mob_no, '1', otp)
    ...
    if request.method == 'POST':
        ...
        if field == 'student':
            if db.execute("SELECT number FROM student WHERE number = :number", {"number": number}).rowcount == 0:
                ...
                raw = db.execute("SELECT password,name FROM student WHERE number = :number", {
            ...
            if db.execute("SELECT number FROM player WHERE number = :number", {"number": number}).rowcount == 0:
                ...
                raw_password = db.execute("SELECT password FROM player WHERE number = :number", {
                                          "number": number}).fetchone()
                password_db = raw_password[0]
                if(password == password_db):
                    player = db.execute(
                        "SELECT * FROM player WHERE number = :ply_no", {"ply_no": number}).fetchone()
                    msg = db.execute("SELECT msg FROM gov_message WHERE receiver = :name", {
                                     "name": 'player'}).fetchall()
                    count = db.execute("SELECT count(msg) FROM gov_message WHERE receiver = :name", {
                                       "name": 'player'}).fetchall()
            ...
            if db.execute("SELECT number FROM applicant WHERE number = :number", {"number": number}).rowcount == 0:
                ...
                raw_password = db.execute("SELECT password FROM applicant WHERE number = :number", {
                                          "number": number}).fetchone()
                password_db = raw_password[0]
                if(password == password_db):
                    applicant = db.execute(
                        "SELECT * FROM applicant WHERE number = :stu_no", {"stu_no": number}).fetchone()
                    msg = db.execute("SELECT msg FROM gov_message WHERE receiver = :name", {
                                     "name": 'applicant'}).fetchall()
                    count = db.execute("SELECT count(msg) FROM gov_message WHERE receiver = :name", {
                                       "name": 'applicant'}).fetchall()
            ...
            if db.execute("SELECT number FROM goverment WHERE number = :number", {"number": number}).rowcount == 0:
                ...
                raw = db.execute("SELECT password,name FROM goverment WHERE number = :number", {
                                 "number": number}).fetchone()
                password_db = raw[0]
                name = raw[1]
                if(password == password_db):
                    applicant = db.execute(
                        "SELECT * FROM applicant WHERE status = 'n'").fetchall()
...
def gov_action(id):
    if request.method == 'POST':
        ...
        if(act == 'accept'):
            db.execute(
                "UPDATE applicant SET status = 'y' WHERE applicant_id = :id", {"id": id})
            db.commit()
            no = db.execute(
                "SELECT number FROM applicant where applicant_id = :id", {"id": id})
            URL = 'http://www.way2sms.com/api/v1/sendCampaign'
            response = sendPostRequest(URL, 'CX97133NP0QNNM9MSNSE0QR7A05GS72A', 'MDV7RXBWBZL593EX',
                                       'stage', no, '1', 'your application is accepted we send interview date soon')
